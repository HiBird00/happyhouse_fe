...
window.onload = function () {
  document.querySelector("#btn-joinus").addEventListener("click", function () {
    window.open("./join-us.html", "mp", "width=400, height=600, left=500, top=300");

  });
  document.querySelector("#btn-notice").addEventListener("click", function () {
    let login = localStorage.getItem("login");
    let user = localStorage.getItem('user');
    if (!login || login === "false" || !user) {
...
$(document).ready(function () {
  ...
  if (localStorage.getItem("login") === "true") {
    $("#welcome").html('<h1>' + user.name + `님 안녕하세요</h1>`);
  } else {
    $("#welcome").html(`<h1>어디로가고싶으세요?</h1>`);
  }
...
function getNotice() {
  ...
  if (notices) {
    ...
    $("#notice-list").empty().append(noticeList);
  } else {
    $("#notice-list").empty().append(`
        <tr>
          <td colspan="2">공지글이 없습니다.</td>
        </tr>`
    );
  }
}
